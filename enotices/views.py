...
import markdown
from enotices.forms import ArticleForm
from enotices.models import Article, ArticleComment, Tag
from por.decorators import ajax_required
...
def _articles(request, articles):
    ...
    return render(request, 'enotices/articles.html', {
        'articles': articles,
...
def article(request, slug):
    ...
    return render(request, 'enotices/article.html', {'article': article})
...
def write(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/enotices/')
    else:
        form = ArticleForm()
    return render(request, 'enotices/write.html', {'form': form})
...
def drafts(request):
    ...
    return render(request, 'enotices/drafts.html', {'drafts': drafts})
...
def edit(request, id):
    ...
    return render(request, 'enotices/edit.html', {'form': form})
        ...
        if request.method == 'POST':
            ...
            for comment in article.get_comments():
                html = '{0}{1}'.format(html, render_to_string(
                    'enotices/partial_article_comment.html',
                    {'comment': comment}))
