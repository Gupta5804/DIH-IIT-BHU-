...
from lan_complain.forms import AnswerForm, QuestionForm
...
def _questions(request, questions, active):
    ...
    return render(request, 'lan_complain/questions.html', {
        'questions': questions,
        'active': active
    })
...
def ask(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.hostel = form.cleaned_data.get('hostel')
            question.description = form.cleaned_data.get('description')
            question.save()
            tags = form.cleaned_data.get('tags')
            question.create_tags(tags)
            return redirect('/lan_complain/')

        else:
            return render(request, 'lan_complain/ask.html', {'form': form})
...
def question(request, pk):
    ...
    return render(request, 'lan_complain/question.html', {
        'question': question,
        'form': form
    })
...
def answer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/lan_complain/{0}/'.format(answer.question.pk))
        else:
